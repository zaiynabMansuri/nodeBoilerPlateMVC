...
async function seedUser () {
  try {
    let user = await User.findOne({
      'username':'Misty_OKeefe9',
      'isActive':true,
      'isDeleted':false
    });
    if (!user || !user.isPasswordMatch('WJjfCBq0kgAwDpK') ) {
      let user = new User({
        'password':'WJjfCBq0kgAwDpK',
        'username':'Misty_OKeefe9',
    ...
    let admin = await User.findOne({
      'username':'Viviane.Bergstrom39',
      'isActive':true,
      'isDeleted':false
    });
    if (!admin || !admin.isPasswordMatch('n2kW0CTSUOtyiJF') ) {
      let admin = new User({
        'password':'n2kW0CTSUOtyiJF',
        'username':'Viviane.Bergstrom39',
...
async function seedUserRole (){
  try {
    ...
    if (user && user.isPasswordMatch('WJjfCBq0kgAwDpK') && userRole){
    ...
    if (admin && admin.isPasswordMatch('n2kW0CTSUOtyiJF') && adminRole){
      let count = await UserRole.countDocuments({
        userId: admin.id,
        roleId: adminRole.id
      });
