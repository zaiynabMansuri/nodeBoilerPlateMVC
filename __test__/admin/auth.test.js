...
beforeAll(async function (){
  try {
    ...
    const db = client.db('dfasasd_test');

    const user = db.collection('user');
    inserted_user = await user.insertOne({
      username: 'Marco_Watsica',
      password: 'ElsNaflteaJRfDP',
      email: 'Leonel.Shields47@yahoo.com',
      name: 'Kirk Baumbach PhD',
      mobileNo: '(246) 979-9580 x45227',
      role: 772,
      resetPasswordLink: {},
      loginRetryLimit: 527,
      loginReactiveTime: '2022-06-28T08:50:27.043Z',
      id: '61f2271b06522bd4cd5fbb0e'
    });
  }
  catch (err) {
...
describe('POST /register -> if email and username is given', () => {
  test('should register a user', async () => {
    let registeredUser = await request(app)
      ...
      .send({
        'username':'Enoch.Denesik',
        'password':'eF5XcVtp0jns49p',
        'email':'Caroline_Denesik@yahoo.com',
        'name':'Rodney Champlin',
        'mobileNo':'392.526.2553 x17855',
...
describe('POST /forgot-password -> if email passed from request body is valid and OTP sent successfully', () => {
  test('should return success message', async () => {
    ...
    let user = await request(app)
      .post('/admin/auth/forgot-password')
      .send({ 'email':'Caroline_Denesik@yahoo.com', });
